...
<?php 
     $db=mysqli_connect('localhost','root','moinak', 'dbms_project');
        $query="SELECT DISTINCT  genre FROM post where visiblity='public'";
...
<?php 
	...
	if($_SERVER['REQUEST_METHOD'] =='POST'){
     ...
     $query="SELECT title FROM readers_paradise where post_id in (select post_id from post where genre='$option_chosen' and visiblity = 'public')";
 else
 		$query="SELECT title FROM readers_paradise where post_id in (select post_id from post where visiblity = 'public')";
 }
...
//Create new session variable 'name'.
